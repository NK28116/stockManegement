...
class EveryStockAnalyzer:
    ...
    def __init__(self, csv_file: str):
        ...
        loggerEveryStockAnalysis.info(f"売買ルールを読み込みました (Version: {rules.meta.version})")

        # ImprovedTradingRules validates rules.is_active internally
    ...
    def save_reports(self, results: List[Dict], is_test_mode: bool = False) -> None:
        ...
        try:
            ...
            detailed_report = self.generate_detailed_report(results)
            detailed_file = (
                f"{config.data_dir}/report/daily/detailed/"
                f"detailed_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
    ...
    def calculate_monthly_returns(self, result: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            loggerEveryStockAnalysis.exception("月次リターン計算に失敗: %s", e)
            return {}

    def get_daily_status(
        self, df: pd.DataFrame, trades: List[Dict[str, any]], days: int = 30
