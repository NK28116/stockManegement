...
class StockChartVisualizer:
    ...
    def visualize_specific_stocks(self, codes: List[str], portfolio_file: str = "my_stock.csv"):
        """指定された銘柄のチャートを作成"""

        print(f"=== 指定銘柄チャート作成開始: {', '.join(codes)} ===")
        print(f"対象ポートフォリオ: {portfolio_file}")

        # Load portfolio (all stocks first, then filter)
        all_stocks = self.load_portfolio_stocks(portfolio_file)
        if not all_stocks:
            print("ポートフォリオが読み込めませんでした")
            return

        # Filter stocks based on provided codes
        target_codes_normalized = [c.replace(".T", "") for c in codes]
        target_stocks = [s for s in all_stocks if s["code"].replace(".T", "") in target_codes_normalized]
        if not target_stocks:
            print(f"指定された銘柄 ({', '.join(codes)}) がポートフォリオに見つかりませんでした。")
            return
        ...
        for i, stock_info in enumerate(target_stocks, 1):
            try:
                print(f"\n[{i}/{len(target_stocks)}] 処理中: {stock_info['name']} ({stock_info['code']})")
        ...
        # Save summary report
        self.save_summary_report(summary_report, portfolio_file)
        # MACD/BBチャートは全銘柄で生成されるため、ここでは呼び出さない
        # self.create_MACD_and_Bollinger_Band_chart(portfolio_file)

        print("\n=== 処理完了 ===")
        print(f"成功: {successful_charts}/{len(target_stocks)} 銘柄")
        print(f"チャート保存先: {os.path.abspath(self.data_dir)}")
